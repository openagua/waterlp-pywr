...
class InstreamFlowRequirement(RiverDomainMixin, PiecewiseLink):
    ...
    def load(cls, data, model):
        ...
        unconstrained_Cost = load_parameter(model, data.pop("counconstrained_Costst", 0.0))
        del (data["type"])
...
class Hydropower(RiverDomainMixin, PiecewiseLink):
    ...
    @classmethod
    def load(cls, data, model):
        base_flow = load_parameter(model, data.pop("base_flow"))
        base_cost = load_parameter(model, data.pop("base_cost"))
